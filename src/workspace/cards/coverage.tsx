...
const read = (props: CardProps['props']) => bundleCoverage(String(props.bundle))

function CoverageMetrics({ props, size }: CardProps) {
  const c = React.useMemo(() => read(props), [props.bundle])
  const page = size === 'page'
  return (
    <Band size={size} cols={5}>
      {page && <Metric size="sm" label="Functions" value={c.functions.length} />}
      <Metric size="sm" label={page ? 'Owned by an agent' : 'Owned'} value={c.owned} deltaTone="ok" />
      ...
      {page && <Metric size="sm" label="Agents serving" value={c.agents.length} hint={`${c.towers.length} tower${c.towers.length === 1 ? '' : 's'}`} />}
    </Band>
  )
}

function CoverageBody({ props, size }: CardProps) {
  const c = React.useMemo(() => read(props), [props.bundle])
              ...
              <Tr key={f.fn.id}>
                ...
                <Td>
                  <div className="flex max-w-[260px] flex-wrap gap-1">
                    {f.owners.map((o) => <Chip key={o.agent.id}>{o.agent.name} · {MODE_LABEL[o.ceiling] ?? o.ceiling}</Chip>)}
                    {!f.owners.length && f.assists.map((o) => <Chip key={o.agent.id}>{o.agent.name}</Chip>)}
                  </div>
                </Td>
              </Tr>
            ))}
...
export const coverageView: ArtifactView = {
  Body: CoverageBody,
  Metrics: CoverageMetrics,
  page: { title: 'Capability coverage' },
}
