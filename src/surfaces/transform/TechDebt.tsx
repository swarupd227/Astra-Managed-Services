...
export function TechDebt() {
  ...
  return (
    <>
      <PageHeader title="Technical debt" subtitle="Operational, architectural, performance, supportability and security debt" />

      <ProducedBy agents={['agt_prospect', 'agt_archivist']} what="attributing demand to debt and costing its interest" />
      ...
      <div className="grid shrink-0 grid-cols-2 gap-4 border-b border-line bg-surface px-4 py-2.5 md:grid-cols-6">
        ...
        <Metric size="sm" label="End of support" value={s.endOfSupport} unit="≤ 180 d" deltaTone={s.endOfSupport ? 'crit' : 'ok'} />
        <Metric size="sm" label="Review overdue" value={s.reviewOverdue} deltaTone={s.reviewOverdue ? 'crit' : 'ok'} />
        <Metric size="sm" label={`${DEBT_CAPACITY.quarter} capacity`} value={`${num(rec.hours)}/${num(DEBT_CAPACITY.hours)}`} unit="h" hint={`${num(rec.relievedHrs)} h/yr relieved`} />
      </div>
